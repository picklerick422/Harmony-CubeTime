...
import BluetoothService, { CubeData } from './BluetoothService';
...
export default class CubeStateManager {
  ...
  getCubeOrientation(): { x: number, y: number, z: number, face: string } {
    // 返回魔方朝向信息
    return {
      x: Math.random() * 360,
      y: Math.random() * 360,
  ...
  getSensorData(): { accelerometer: number[], gyroscope: number[] } {
    return {
      accelerometer: [
        Math.random() * 2 - 1,
        Math.random() * 2 - 1,
